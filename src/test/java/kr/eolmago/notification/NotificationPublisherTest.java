...
class NotificationPublisherTest {
	...
	@Test
	@DisplayName("알림 발행: 저장 후 SSE push")
	void givenCommand_whenPublish_thenSaveAndPush() {
		// given
		NotificationRepository notificationRepository = mock(NotificationRepository.class);
		UserRepository userRepository = mock(UserRepository.class);
		NotificationSseHub sseHub = mock(NotificationSseHub.class);

		NotificationPublisher sut = new NotificationPublisher(notificationRepository, userRepository, sseHub);
		...
		then(sseHub).should().push(eq(userId), captor.capture());
